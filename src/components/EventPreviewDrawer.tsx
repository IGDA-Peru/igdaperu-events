import { CalendarDays, Clock3, ExternalLink, MapPin, Users, X } from 'lucide-react'
...
export function EventPreviewDrawer({ event, onClose }: { event: EventItem | null; onClose: () => void }) {
  ...
  return createPortal(
    <div className="event-preview-layer" role="presentation" onMouseDown={(mouseEvent) => { if (mouseEvent.target === mouseEvent.currentTarget) onClose() }}>
      <aside className="event-preview-drawer" role="dialog" aria-modal="true" aria-labelledby="event-preview-title">
        ...
        <div className="event-preview-meta">
          ...
          <div><Users size={19} aria-hidden="true" /><span><strong>Organiza</strong><Link to={`/comunidades/${event.communitySlug}`} onClick={onClose}>{event.communityName}</Link></span></div>
        </div>
