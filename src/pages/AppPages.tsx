...
export function EventEditorPage() {
  ...
  const todayDateKey = limaTodayDateKey()
  ...
  const summaryMilestoneLabel = !minimumReady ? 'Borrador / Solo Comunidades' : form.visibility === 'network' ? 'Solo Comunidades' : 'Para publicar'
        ...
        <form className="event-editor-card" onSubmit={(event) => void save(event)}>
          ...
          {activeSection === 'datetime' && <section className="editor-section" id="editor-datetime" role="tabpanel" aria-labelledby="editor-tab-datetime" tabIndex={-1}>
            ...
            <div className="form-grid schedule-dates">
              ...
              {schedule.mode === 'range' && <label className="editor-field"><FieldLabel required>Fecha de fin</FieldLabel><input type="date" aria-invalid={Boolean(fieldErrors.endsAt)} aria-describedby={fieldErrors.endsAt ? 'event-end-error event-end-help' : 'event-end-help'} value={schedule.endDate} min={schedule.startDate > todayDateKey ? schedule.startDate : todayDateKey} onChange={(event) => updateSchedule({ endDate: event.target.value })} /><small className="field-help" id="event-end-help">Desde hoy · Incluye este día en el evento.</small><FieldError id="event-end-error" message={fieldErrors.endsAt} /></label>}
            </div>
          ...
          {activeSection === 'location' && <section className="editor-section" id="editor-location" role="tabpanel" aria-labelledby="editor-tab-location" tabIndex={-1}>
            ...
            {needsPhysicalLocation && <>
              ...
              {form.locationPrecision === 'exact' && <div className="location-editor-block">
                <div className="location-search-heading"><h3><FieldLabel required>Ubicación exacta</FieldLabel></h3><p className="field-help">Busca un lugar, haz clic en el mapa para seleccionar una dirección o escríbela manualmente.</p></div>
...
export function PlatformAdminPage() {
  ...
  const approvedCommunities = communities.filter((community) => community.status === 'approved').length
  const pendingCommunities = communities.filter((community) => community.status === 'pending').length
  return <div className="dashboard-page platform-admin-page">
    <PanelTitle title="Administración IGDA" description="Aprueba comunidades y conserva el control de la red." action={<Link className="secondary-button" to="/app/admin/propuestas"><Send size={16} /> Propuestas de eventos</Link>} />
    <section className="admin-summary" aria-label="Resumen administrativo">
      <article className="admin-stat admin-stat--communities"><span className="admin-stat-icon"><Users size={18} /></span><div><span>Comunidades activas</span><strong>{approvedCommunities}</strong><small>{communities.length} registradas en total</small></div></article>
      <article className={`admin-stat ${pendingCommunities ? 'admin-stat--attention' : ''}`}><span className="admin-stat-icon"><Clock3 size={18} /></span><div><span>Por revisar</span><strong>{pendingCommunities}</strong><small>{pendingCommunities ? 'Comunidades pendientes de aprobación' : 'No hay comunidades pendientes'}</small></div></article>
      <article className={`admin-stat ${reports.length ? 'admin-stat--alert' : ''}`}><span className="admin-stat-icon"><CircleAlert size={18} /></span><div><span>Reportes pendientes</span><strong>{reports.length}</strong><small>{reports.length ? 'Requieren una revisión' : 'Todo está al día'}</small></div></article>
    </section>
    <section className="admin-create">
      <div className="admin-section-heading"><div><span className="eyebrow">Acción rápida</span><h2>Nueva comunidad</h2><p className="muted-copy">Registra una comunidad para comenzar su revisión y aprobación.</p></div></div>
      <form onSubmit={create}><input aria-label="Nombre de la nueva comunidad" placeholder="Nombre de la nueva comunidad" value={newName} onChange={(event) => setNewName(event.target.value)} /><button className="primary-button"><Plus size={16} /> Crear comunidad</button></form>{error && <FormError message={error} />}
    </section>
    <section className="settings-section admin-tools"><div className="admin-tools-heading"><div><span className="eyebrow">Herramientas de red</span><h2>Mantenimiento y sincronización</h2><p className="muted-copy">Actualiza las fuentes externas y conserva los recursos visuales optimizados.</p></div></div><div className="admin-tool-grid"><article className="admin-tool-card"><div className="admin-tool-card-copy"><span className="admin-tool-icon"><RefreshCw size={18} /></span><div><h3>Google Sheets</h3><p>Importa comunidades desde la pestaña <strong>TO NOTION</strong> con validación activa.</p></div></div><button className="primary-button" type="button" onClick={() => void sync()} disabled={syncing}><RefreshCw size={16} className={syncing ? 'spin' : ''} /> {syncing ? 'Actualizando…' : 'Actualizar comunidades'}</button>{syncError && <FormError message={syncError} />}{syncResult && <div className="sync-result" role="status"><strong>Actualización completada</strong><span>{syncResult.created} creadas · {syncResult.updated} actualizadas · {syncResult.skipped} omitidas</span>{syncResult.skippedRows.length > 0 && <details><summary>Ver filas omitidas</summary><ul>{syncResult.skippedRows.map((row, index) => <li key={`${row.sourceId || 'row'}-${row.row}-${index}`}>{row.name || row.sourceId || `Fila ${row.row}`}: {row.reason}</li>)}</ul></details>}</div>}</article><article className="admin-tool-card"><div className="admin-tool-card-copy"><span className="admin-tool-icon"><CalendarDays size={18} /></span><div><h3>Google Calendar</h3><p>Publica eventos aprobados y retira automáticamente los que ya no deben aparecer.</p></div></div><button className="primary-button" type="button" onClick={() => void syncCalendar()} disabled={calendarSyncing}><CalendarDays size={16} className={calendarSyncing ? 'spin' : ''} /> {calendarSyncing ? 'Sincronizando…' : 'Sincronizar calendario'}</button>{calendarSyncError && <FormError message={calendarSyncError} />}{calendarSyncResult && <div className="sync-result" role="status"><strong>Calendario actualizado</strong><span>{calendarSyncResult.created} creados · {calendarSyncResult.updated} actualizados · {calendarSyncResult.removed} retirados{calendarSyncResult.errors ? ` · ${calendarSyncResult.errors} con error` : ''}</span>{calendarSyncResult.errorItems.length > 0 && <details><summary>Ver errores</summary><ul>{calendarSyncResult.errorItems.map((item, index) => <li key={`${item.eventId || 'event'}-${index}`}>{item.eventId || 'Evento'}: {item.message}</li>)}</ul></details>}</div>}</article><article className="admin-tool-card"><div className="admin-tool-card-copy"><span className="admin-tool-icon"><ImagePlus size={18} /></span><div><h3>Optimizar imágenes</h3><p>Convierte logos y banners existentes a WebP. Los recursos que ya están en WebP se omiten.</p></div></div><button className="primary-button" type="button" onClick={() => void migrateAssets()} disabled={assetMigration.status === 'running'}><RefreshCw size={16} className={assetMigration.status === 'running' ? 'spin' : ''} /> {assetMigration.status === 'running' ? 'Optimizando…' : 'Optimizar recursos'}</button>{assetMigration.status === 'running' && assetMigration.total > 0 && <div className="admin-tool-progress" role="status"><div className="admin-tool-progress-label"><span>{assetMigration.label || 'Revisando recursos…'}</span><strong>{assetMigration.completed}/{assetMigration.total}</strong></div><progress value={assetMigration.completed} max={assetMigration.total} /></div>}{assetMigration.error && <FormError message={assetMigration.error} />}{assetMigration.result && <div className="sync-result" role="status"><strong>Imágenes revisadas</strong><span>{assetMigration.result.migrated} optimizadas · {assetMigration.result.skipped} omitidas · {assetMigration.result.failed} con error</span>{assetMigration.result.errors.length > 0 && <details><summary>Ver errores</summary><ul>{assetMigration.result.errors.map((item, index) => <li key={`${item.label}-${index}`}>{item.label}: {item.message}</li>)}</ul></details>}</div>}</article></div><p className="muted-copy admin-tools-note">La migración de imágenes solo procesa archivos actualmente referenciados por comunidades y eventos.</p></section>
    {loading ? <LoadingState label="Cargando comunidades" /> : <div className="admin-management-grid"><section className="admin-management-card admin-communities"><div className="admin-section-heading"><div><span className="eyebrow">Moderación</span><h2>Comunidades</h2><p className="muted-copy">Aprueba, suspende o reactiva comunidades de la red.</p></div><strong className="admin-section-count">{communities.length}</strong></div><div className="admin-list">{communities.map((community) => <div className="admin-row" key={community.id}><div><strong>{community.name}</strong><small>{community.slug} · {community.status}</small></div><div className="row-actions">{community.status === 'pending' && <button className="secondary-button" type="button" onClick={() => void moderate(community, 'approved')}><Check size={16} /> Aprobar</button>}{community.status === 'approved' && <button className="icon-button danger" type="button" onClick={() => void moderate(community, 'suspended')} aria-label={`Suspender ${community.name}`}><X size={17} /></button>}{community.status === 'suspended' && <button className="secondary-button" type="button" onClick={() => void moderate(community, 'approved')}>Reactivar</button>}</div></div>)}</div></section><section className="settings-section admin-reports"><div className="admin-section-heading"><div><span className="eyebrow">Seguridad y calidad</span><h2>Reportes pendientes</h2><p className="muted-copy">Revisa incidencias informadas sobre los eventos publicados.</p></div><strong className="admin-section-count">{reports.length}</strong></div>{reports.length ? <div className="report-list">{reports.map((report) => <div className="report-row" key={report.id}><div><strong>{report.eventTitle}</strong><p>{report.reason}</p></div><button className="secondary-button" type="button" onClick={() => void resolveEventReport(report.id).then(load)}>Marcar revisado</button></div>)}</div> : <p className="muted-copy admin-empty-copy">No hay reportes pendientes.</p>}</section></div>}
  </div>
}
