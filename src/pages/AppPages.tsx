import { Check, ChevronRight, Clipboard, Edit3, Mail, Plus, RefreshCw, Shield, Users, X } from 'lucide-react'
...
import { archiveEvent, createCommunity, listCommunities, listEventReports, listManagedEvents, resolveEventReport, saveEvent, syncCommunitiesFromSheet, updateCommunity, updateCommunityStatus } from '../lib/data'
import { slugify } from '../lib/format'
import { supabase } from '../lib/supabase'
import type { Community, CommunitySyncResult, EventInput, EventItem, EventReport, Role } from '../types'
...
export function DashboardPage() {
  ...
  return <div className="dashboard-page"><PanelTitle title={`Hola${profile?.displayName ? `, ${profile.displayName}` : ''}`} description="Este es tu espacio para consultar y administrar tus eventos." action={<Link className="primary-button" to={canManage ? '/app/eventos/nuevo' : '/'}><Plus size={17} /> {canManage ? 'Nuevo evento' : 'Volver a eventos'}</Link>} />{!configured && <div className="setup-panel"><Shield size={22} /><div><strong>Supabase aún no está conectado</strong><p>El panel está listo, pero necesitas configurar las variables de entorno para activar tus datos y permisos reales.</p></div></div>}<div className="dashboard-grid"><section className="dashboard-main"><div className="subheading"><h2>Tus eventos</h2>{canManage && <Link to="/app/eventos">Ver todos <ChevronRight size={17} /></Link>}</div>{loading ? <LoadingState label="Cargando tus eventos" /> : events.length ? <div className="event-list">{events.slice(0, 5).map((event) => <EventCard event={event} compact showVisibility key={event.id} />)}</div> : <EmptyEvents authenticated />}</section><aside className="dashboard-sidebar"><h2>Tus comunidades</h2>{memberships.length ? <div className="membership-list">{memberships.map((membership) => <div className="membership-row" key={membership.communityId}><span className="membership-avatar"><Users size={18} /></span><span><strong>{membership.communityName}</strong><small>{roleLabel(membership.role)}</small></span></div>)}</div> : <p className="muted-copy">Aún no tienes permisos de gestión. Puedes seguir consultando los eventos de la red.</p>}{canManage && <Link className="secondary-button full" to="/app/comunidad">Gestionar comunidades</Link>}{roles.includes('platform_admin') && <Link className="secondary-button full" to="/app/admin">Administración IGDA</Link>}</aside></div>{user && <p className="account-caption">Sesión iniciada como {user.email}</p>}</div>
}
...
export function ManagedEventsPage() {
  ...
  return <div className="dashboard-page"><PanelTitle title="Tus eventos" description="Crea, publica y actualiza los eventos de tus comunidades." action={<Link className="primary-button" to="/app/eventos/nuevo"><Plus size={17} /> Nuevo evento</Link>} />{message && <p className="form-message success">{message}</p>}{loading ? <LoadingState label="Cargando eventos" /> : events.length ? <div className="managed-event-list">{events.map((event) => <div className="managed-event-row" key={event.id}><EventCard event={event} compact showVisibility /><span className={`status-label ${event.status}`}>{event.status === 'published' ? 'Publicado' : event.status === 'draft' ? 'Borrador' : 'Archivado'}</span><div className="row-actions"><Link className="icon-button" to={`/app/eventos/${event.id}`} aria-label={`Editar ${event.title}`}><Edit3 size={17} /></Link>{event.status !== 'archived' && <button className="icon-button danger" type="button" onClick={() => void archive(event.id)} aria-label={`Archivar ${event.title}`}><X size={17} /></button>}</div></div>)}</div> : <EmptyEvents authenticated />}</div>
}
